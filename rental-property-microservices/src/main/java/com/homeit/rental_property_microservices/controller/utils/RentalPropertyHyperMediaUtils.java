package com.homeit.rental_property_microservices.controller.utils;

import com.homeit.rental_property_microservices.controller.RentalPropertyControllerV2;
...
public class RentalPropertyHyperMediaUtils {
    ...
    public RentalPropertyDescriptor describeRentalProperty(RentalPropertyDTO rentalPropertyDTO) {
    ...
    public PropertiesCollectionDescriptor describeRentalPropertyCollection(List<RentalPropertyDTO> allProperties) {

        if(allProperties.isEmpty()) return null;

        List<RentalPropertyDescriptor> parsedProperties = parseProperties(allProperties);
    ...
    private List<RentalPropertyDescriptor> parseProperties(List<RentalPropertyDTO> allProperties) {
        return allProperties.stream()
            .map(this::describeRentalProperty).toList();
    }

    public void addAllPropertiesLink( PropertiesCollectionDescriptor collectionDescriptorDTO) {
