package com.homeit.revoke_token_service;
...
class IntegrationTestsApplicationTests {
	...
	private void tenantFailsAtCreatingProperty(CreatedUser createdTenant) {
		...
		failsWhenCreatingProperty("John Doe Landlord", createdTenant.userId(), createdTenant.userAccessToken(), 403);
	}
	...
	private void landlordFailsAtCreatingProperty(String landlordId, String landlordToken) {
		...
		RestAssured.baseURI = "http://localhost/";
		RestAssured.port = 8080;
		failsWhenCreatingProperty("John Doe Landlord", landlordId, landlordToken, 401);
	}
	...
	private void failsWhenCreatingProperty(String landlordName, String landlordId, String landlordToken, int expectedStatus) {
		given()
				.contentType("application/json")
				.body(createPropertyPayload(landlordName, landlordId)).
				header("Authorization", "Bearer " + landlordToken) // Add the Bearer token here
				...
				then()
				.log().all()
				.statusCode(equalTo(expectedStatus));
	}
